    ...
    <section id="faq" className="py-10 sm:py-16 md:py-24">
      <div className="mx-auto max-w-7xl px-4">
        <div className="mb-8 md:mb-12 text-center">
          <h3 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl md:text-5xl mb-4">
            {dict.faq.title_part1} <span className="text-primary">{dict.faq.title_part2}</span>
          </h3>
          <p className="mx-auto max-w-3xl text-base md:text-lg text-muted-foreground">{dict.faq.subtitle}</p>
        </div>
...
function FaqItem({ question, answer, isOpen, onClick }: FaqItemProps) {
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-primary hover:ring-1 hover:ring-primary/20 group bg-card/20 backdrop-blur-sm">
      ...
      >
        <h3 className="text-sm md:text-base font-medium text-card-foreground group-hover:text-primary transition-colors duration-300">
          {question}
        </h3>
        <ChevronDown
      ...
      >
        <p className="text-sm md:text-base text-muted-foreground pb-2">{answer}</p>
      </div>
    </div>
  )
}
